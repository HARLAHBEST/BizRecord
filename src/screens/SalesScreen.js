...
export default function SalesScreen({ navigation }) {
  ...
  const sendWhatsAppReceipt = (phone, name, receiptUrl) => {
    ...
    const message = `Hello ${name}, here is your sales receipt.\n${
      receiptUrl ? `View or download: ${receiptUrl}` : ''
    }`;
  ...
  const renderSyncBadge = (item) => {
    ...
    if (item.sync_status === 'failed') {
      return (
        <Text style={{ color: '#E53935', fontSize: 11, marginTop: 6 }}>
          Sync failed
        </Text>
      );
    }
  ...
  const salesSummary = useMemo(() => {
    ...
    const pendingSync = sales.filter(
      (item) => item?.sync_status && item.sync_status !== 'synced',
    ).length;
    return {
      totalAmount,
      totalQuantity,
      ...
      {!loading ? (
        ...
        >
          <Card style={{ marginTop: 6 }}>
            ...
            <Subtle>
              {salesSummary.count} record(s) | {salesSummary.totalQuantity} item(s)
              sold
            </Subtle>
            <Subtle style={{ marginTop: 4 }}>
              Revenue: {formatCurrency(salesSummary.totalAmount)}
            </Subtle>
            {salesSummary.pendingSync > 0 ? (
              <Subtle style={{ marginTop: 4 }}>
                {salesSummary.pendingSync} sale record(s) waiting to sync
              </Subtle>
            ) : null}
          </Card>
        </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item?.id ? String(item.id) : `sale-${index}`
          }
            ...
            <Card style={[styles.itemCard, { marginBottom: 14 }]}>
              ...
              >
                <View style={{ flex: 1, paddingRight: 8 }}>
                  ...
                  <Subtle>{formatDateTime(item.createdAt)}</Subtle>
                  <Subtle>
